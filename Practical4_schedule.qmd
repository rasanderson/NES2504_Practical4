...
* **Count data** Number of birds in different gardens, counts of pollinators visiting flowers, number of muscle reflexes in response to electical stimuli, number of lobster retrieved from different inland water areas etc.
* **Positive numbers only** Heights of oak trees, weights of brown rats, yield of crops, mean length of haddock landed etc. These numbers might have decimal places, but they cannot be negative
...
### Before you begin
1. Start RStudio in the usual way. Open your `NES2504` project from the main menu by clicking on **File -> Open project...** and navigate to your NES2504 project file and open it. Check that the word "None" has been replaced by "NES2504" in the project listing at the top right of the RStudio window.
2. From the main menu click "File -> New file -> R Script"
3. Save your script, e.g. call it `practical4.R`

### Data for this practical
Go to Canvas and download the **lichen.csv**,  **sppcount.csv** and **beetle.csv** data sets. Start up your "File Explorer" and move the files from your **Downloads** folder to either your **NES2504** folder, or your **NES2504/Data** folder depending on your preferences.

### Initial lines in your script
As usual, begin your script with a useful comment, and then load the `mosaic` and `car` packages, as shown below:
...
### Import data from .csv files into R 
Now import the three .csv files you downloaded from Canvas into R, `lichen.csv`, `sppcount.csv` and `beetle.csv`. Add the following code, plus suitable comments, to your `practical4.R` script. **Note**: The code below assumes you have a `Data` subfolder inside your `NES2504` folder. If you have stored your CSV files directly in your `NES2303` folder then omit the `Data/` from below.
...
* finally `gf_theme()` to refine your graph using a different theme such as `theme_bw()` or as here `theme_classic()`
...
As you would expect, the model is highly significant, with a negative (significant) gradient for slope of `r round(summary(sppcount_lm)$coef["pollution","Estimate"],2)`, in other words for every 1 mg/l increase in pollution there is a decline in the number of species by about `r abs(round(summary(sppcount_lm)$coef["pollution","Estimate"],2))`. When there is no pollution the linear model estimates the number of species to be about about `r round(summary(sppcount_lm)$coef["(Intercept)","Estimate"],2)`, which looks a big underestimate compared to your graph, when the horizontal axis of pollution is at zero.  So you have identified at least two problems with the linear models, at low and high pollution levels.

### Have model assumptions been broken?
You will recall that linear models assume your data (or more precisely, the noise or errors around your fitted line) come from a normal or Gaussian distribution. The easiest way to check this is via residual and QQ plots. Remember to click in the Console window (bottom left) and hit the big 'Enter' key :
...
You can immediately see problems in both plots. The residuals vs fitted plots shows a U-shaped line, whereas ideally it should be roughly horizontal, whilst the QQ has much high observed (standardized) residuals at high values of the predicted, whereas ideally all the points should be roughly on the dotted diagonal. Both graphs suggest problems with the simple model and its assumptions are not satisfied.

### What are the predicted numbers of species in heavily polluted rivers?
Hopefully you have already detected that this is a fundamental problem, simply by looking at your original graph of the data, with the straight line of the number of species added. What, for example, is the predicted number of species when water pollution is at 7 mg/l ?  You don't actually have any observations at 7 mg/l but let's see what your model predicts:
...
# Obtain predicted value at 7 mg/l
lm_species_predictor(7)
...
* The **coefficients** are your **explanatory** variables (i.e. pollution). You may notice that there are two coefficients when our model only specified one: `(Intercept)` and `pollution`.

 - `(Intercept)` As usual, this is the predicted number of species when your explanatory variable (pollution) is zero. Here the estimated value for this is `r round(summary(sppcount_glm)$coefficients["(Intercept)", "Estimate"],3)`, which seems very low, but keep reading.
 - `pollution` this is the change in your number of species with increasing pollution, which is negative, reflecting a decline in biodiversity with pollution. However, the value of `r round(summary(sppcount_glm)$coefficients["pollution", "Estimate"], 3)` seems too low. Again, keep reading.
- instead of t-statistics, maximum likelihood methods such as GLM produce z-statistics, which can be interpreted in a similar way to produce p-values. Both p-values are extremely low. Remember that a value such as `7.79-e13` is shorthand for `0.000000000000779` (13 zeroes) whilst `<2e-16` indicates less than `0.0000000000000002` (16 zeroes). You write such values `p<0.001` in a report.

* The **residual deviance** is a measure of how good your model is. If your residual deviance value is close to zero, then the model is  very good. 
...
There is a lot of information in the output and it is easy to get overwhelmed. The main things to note are the `Estimate` values, and when you report them in a table you would typically quote their associated z and p statistics, along with the overall AIC. Another useful thing is to assess the percentage variation explained by your model:

$$ \text{%  explained by model} = \frac{\text{Null deviance} - \text{Residual deviance}}{\text{Null deviance}}*100$$

Now you can calculate the percentage variation explained by your model above, based on the output of your summary table. Remember to use brackets around the subtraction term.

## What about the strange Estimates from the GLM?
As we noted above, the values of `r round(summary(sppcount_glm)$coefficients["(Intercept)", "Estimate"],2)` and `r round(summary(sppcount_glm)$coefficients["pollution", "Estimate"], 3)` do not quite match what was expected. Indeed, looking at your original graph, an estimated number of species of only `r round(summary(sppcount_glm)$coefficients["(Intercept)", "Estimate"],2)` when the water is unpolluted (x-axis at zero) does not match your graph where it looks to be around 29 or 30 species! This is because GLMs use what is known as a "link function" in their calculations; if you are interested see the [How to generalise page from the interactive website (right-click open in new tab)](https://naturalandenvironmentalscience.shinyapps.io/generalised_linear_models/#section-how-to-generalise). Poisson GLMs use natural logarithms as their link function, so we would need to take the antilogarithm to get back to the original units. As these are natural logarithms, (i.e. $log_e$ rather than $log_{10}$) we need to take the exponential to convert to the original units, available through the `exp()` function. Let's check with the expected number of species when pollution is zero, i.e. the intercept:

```{r}
exp(3.154)
```

That looks more sensible based on our original scatterplot of the data. To finish off, we will check the model assumptions, predict the number of species when pollution is 7 mg/l and replot our original graph, but with a GLM rather than linear model.

## Check Poisson GLM model assumptions
As before, create a QQ plot of the residuals. Use the `plot()` function and specify `which = 1:2` as parameters, as earlier. Remember to click in the Console window (bottom left) and hit enter to see each plot. You should end up with graphs similar to these:
...
This is much better. The red line in the residuals v fitted plot is roughly horizontal, and in the QQ plot most points are roughly on the diagonal. This indicates that the Poisson GLM is a much more appropriate model for these data than the linear model.

## What is the predicted number of species in heavily polluted streams with the GLM?
With the linear model we had the problem that our model could predict negative numbers of species in polluted water. What do we get with the GLM? Modify your `makeFun()` call to create a new `glm_species_predictor()` function, and see if you get this value for 7 mg/l pollution?

```{r, echo=FALSE}
# Create a predictor function for GLM Poisson model
glm_species_predictor <- makeFun(sppcount_glm)

# Obtain predicted value at 7 mg/l
glm_species_predictor(7)

```

The biodiversity is obviously very low, close to zero, but it is still positive, which makes more sense than the negative number of species estimated by the incorrect linear model for 7 mg/l pollution we used in Section 1.3. It also looks right based on your original scatterplot.

## Modify scatterplot of original data to show Poisson GLM
...
```{r}
# Scatterplot (point plot) of count of species with GLM poisson curve
gf_point(species ~ pollution, data=sppcount_dat) %>% 
  gf_labs(x = "Aquatic pollution level (mg/l)", y="Number of invertebrate species") %>% 
...
## Summary statistics and visualisation of lichen data
...
```{r}
# Force R to recognise that lichen 0/1 is presence/absence not continuous
...
```{r}
# Boxplot to show impact of calcium on lichen presence/absence
...
### Two ways to update your (linear or) generalised linear model
When you have the results of a model with one or more interaction terms, the best strategy is to:

* refit the model without the most complicated non-significant interaction term (here simply the `airpoll:calcium` interaction term)
* look at the `summary()` of the model
* see if the simpler model is as good as the more complex one (models can be formally compared if needed)
* repeat if required
...
```{r, eval=FALSE}
# Revised simpler model written with just main effects
...
```{r}
# Revised simpler model by updating lichen_glm1 and deleting interaction
...
Notice now that the main effects for air pollution and calcium are both significant after removing the interaction term. Also, the **AIC** value is lower in the second model meaning this model is an improvement over the more complex one we ran earlier. What do the results of this simpler model tell you about lichen and its relationship with calcium and air pollution? Which have positive or negative effects on the presence of lichens?

The better of the two models is the second, simpler one, without the interaction term. A good general rule is 'KISS' (Keep it Simple Stupid) as it is easy to create too complex a model to isn't biologically meaningful. Finally, look at the outputs of `summary(lichen_glm2)` : what is the percentage variation explained by this model?

### Checking binomial model assumptions and making predictions
...
```{r}
# Jittered scatterplot of lichen presence/absence plus GLM curve
...
Add 2 additional R lines to the code to modify the axis labels (`gf_labs`) and overall plotting theme (`gf_theme`). Then add code (copy and paste and edit previous set) to create a second plot of calcium vs air pollution.
...
## How to include sample size in your GLM
...
```{r}
# Calculate numbers of surviving beetles
...
```{r}
# GLM that accounts for different sample sizes in each row
beetle_glm <- glm(cbind(Number_killed, Number_alive) ~ Dose, family=binomial(link = "logit"), data=beetle_dat)
summary(beetle_glm)

```

The output shows, as expected, a highly significant effect of the pesticide on the survivorship of the beetles. Again, you might be puzzled by the `r round(summary(beetle_glm)$coefficients["(Intercept)" , "Estimate"], 3)` value for the Intercept, which is the mortality at zero dose of insecticide. This is because binomial GLMs use the "logit" link function which are natural logarithms of the odds of success to failure. This can be calculated manually (which is a bit tricky) so it is much easier to use the `makeFun()` approach:

```{r}
# Create function to predict numbers of beetles and predict for 0 pesticide dose
...
## Plotting GLM results when sample size varies
